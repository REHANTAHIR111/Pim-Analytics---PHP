...
$perm = "SELECT * FROM permission JOIN modules ON modules.id = permission.module_id WHERE role_id = $role_id AND module_id = 5";
$result = mysqli_query($conn, $perm);
$row = mysqli_fetch_assoc($result);

if (!$row) {
    die("❌ No permission entry found for role_id = $role_id and module_id = 5");
}
...
$condition = ($can_view_all == 1) ? "" : "WHERE brand.creator_id = $user_id";

$query = "SELECT id, name, sorting, website_image, mobile_image, status from brand $condition ORDER BY brand.id DESC";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $action .= '<a href="/php/pim/pages/brand/view.php?id=' . $row['id'] . '">
    ...
    if ($edit) {
        $action .= '<a href="/php/pim/pages/brand/edit.php?id=' . $row['id'] . '">
    ...
    if ($delete){
        $action .= '<a href="/php/pim/pages/brand/delete.php?id=' . $row['id'] . '">
    ...
    $users[] = [$id, $name, $sorting, $image, $mobile_image, $status ,$action];
}
?>