...
$perm = "SELECT * FROM permission 
         ...
         WHERE role_id = $role_id AND module_id = 3";
$result = mysqli_query($conn, $perm);

$row = mysqli_fetch_assoc($result);

if (!$row) {
    die("❌ No permission entry found for role_id = $role_id and module_id = 3");
}
...
$condition = ($can_view_all == 1) ? "" : "WHERE product.creator_id = $user_id";

$query = "SELECT product.*, 
                 GROUP_CONCAT(productcategories.name SEPARATOR ', ') AS category 
          FROM product 
          LEFT JOIN product_categories ON product_categories.product_id = product.id 
          LEFT JOIN productcategories ON productcategories.id = product_categories.category_id 
          $condition 
          GROUP BY product.id 
...
while ($row = mysqli_fetch_assoc($result)) {
    $id = $row['id'];
    $image = $row['upload_featured_image'];
    $name = $row['name'];
    $ln_sku = $row['ln_sku'];
    $price = $row['regular_price'] == 0 ? '' : $row['regular_price'];
    ...
    $enable_pre_order = $row['enable_pre_order'] == 0 ? '<div class="text-danger font-semibold">Disabled</div>' : '<div class="text-success font-semibold">Enabled</div>';
    $action = '<div style="display: flex; gap: 10px; align-items: center;">';

    // Always show the "view" button
    $action .= '<a href="/php/pim/pages/product/view.php?id=' . $row['id'] . '">
    ...
    if ($edit) {
        $action .= '<a href="/php/pim/pages/product/edit.php?id=' . $row['id'] . '">
    ...
    if ($delete){
        $action .= '<a href="/php/pim/pages/product/delete.php?id=' . $row['id'] . '">
    ...
    $users[] = [$id, $image, $name ,$ln_sku, $price, $sale_price, $category, $notes ,$status, $enable_pre_order, $action];
}
?>