<?php 
    ...
    $old = $_SESSION['old'] ?? [];
    $isEdit = $_SESSION['edit_mode'] ?? false;
    $editId = $_SESSION['edit_id'] ?? null;
    ...
    if (!$showEditModal && $editId) {
        $id = $editId;
        $result = mysqli_query($conn, "SELECT * FROM product_faqs WHERE id = $editId");
    ...
    $tiR = $errors['title'] ?? '';
    $qeR = $errors['question_english'] ?? '';
    $qaR = $errors['question_arabic'] ?? '';
    $aeR = $errors['answer_english'] ?? '';
    $aaR = $errors['answer_arabic'] ?? '';


    unset($_SESSION['errors'], $_SESSION['old'], $_SESSION['edit_mode'], $_SESSION['edit_id']);
    ...
    $perm = "SELECT * FROM permission JOIN modules ON modules.id = permission.module_id WHERE role_id = $role_id AND module_id = 7";
    $result = mysqli_query($conn, $perm);

    $row = mysqli_fetch_assoc($result);

    if (!$row) {
        die("❌ No permission entry found for role_id = $role_id and module_id = 7");
    }
    ...
    $condition = ($can_view_all == 1) ? "" : "WHERE product_faqs.creator_id = $user_id";
    $query = "SELECT * FROM product_faqs $condition ORDER BY product_faqs.id DESC";

    $result = mysqli_query($conn, $query);

    //show all tag records into listing//
    while ($row = mysqli_fetch_assoc($result)) {
        $id = $row['id'];
        $title = $row['title'];
        $question_english = $row['question_english'];
        $question_arabic = $row['question_arabic'];
        $answer_english = $row['answer_english'];
        $answer_arabic = $row['answer_arabic'];
        ...
        $action .= '<button class="view" onclick=\'viewData(' . json_encode($row, JSON_HEX_APOS | JSON_HEX_QUOT) . ')\'>
        ...
        if ($edit) {
            $action .= '<button class="edit" onclick=\'editData(' . json_encode($row, JSON_HEX_APOS | JSON_HEX_QUOT) . ')\'>
        ...
        if ($delete){
            $action .= '<a href="/php/pim/pages/productFaqs/delete.php?id=' . $row['id'] . '">
        ...
        $faqs[] = [$id, $title, $question_english, $question_arabic, $answer_english, $answer_arabic, $status, $action];
    }
?>
...
<div id="error-message"
    class="flex items-start justify-center p-6 rounded text-danger bg-danger dark:bg-danger-dark-light fixed w-80 shadow-md shadow-danger"
    style='right:12px; min-height: 70px; top:70px; z-Index:999999;'>
...
<div x-data="custom">
    <div class="panel">
        <div class="flex gap-3 items-center absolute top-5">
            ...
            <?php endif; ?>
            <div 
                x-data="{ showAddModal: <?= $showAddModal ? 'true' : 'false' ?> }"
            >
                <?php if ($create) : ?>
                    <button @click="showAddModal = true" class="btn btn-dark">Create Products FAQS</button>
                <?php endif; ?>
                <!-- Modal Backdrop -->
                <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto items-center justify-center"
                    :class="showAddModal && '!block'">
                    <div class="flex items-center justify-center min-h-screen px-4">
                        <div class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-5xl my-8 text-black dark:text-white-dark animate__animated"
                            ...
                            <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                                <h5 class="font-bold text-lg">Add Product FAQ'S</h5>
                                <button type="button" class="text-white-dark hover:text-dark"
                                    @click="showAddModal = false; window.location.reload();">
                            ...
                            <div class="p-5">
                                <form action="./action.php" method="POST">
                                    <div>
                                        ...
                                        <small style="color: #b91c1c;" class="mt-1"><?= $tiR ?></small>
                                    </div>
                                    <div class="grid grid-cols-2 w-full mt-3 gap-3">
                                        <div>
                                            <label>Question Englsih</label>
                                            <input type="text" class="form-input" name="question_english"
                                                value="<?= $old['question_english'] ?? '' ?>"
                                                placeholder="Enter Question English">
                                            <small style="color: #b91c1c;" class="mt-1"><?= $qeR ?></small>
                                        </div>
                                        <div>
                                            <label>Question Arabic</label>
                                            <input type="text" class="form-input" name="question_arabic"
                                                value="<?= $old['question_arabic'] ?? '' ?>"
                                                placeholder="Enter Question Arabic">
                                            <small style="color: #b91c1c;" class="mt-1"><?= $qaR ?></small>
                                        </div>
                                        <div>
                                            <label>Answer Englsih</label>
                                            <textarea name="answer_english" id="answer_english"
                                                class="form-textarea h-20"
                                                placeholder="Enter Answer English"><?= $old['answer_english'] ?? '' ?></textarea>
                                            <small style="color: #b91c1c;" class="mt-1"><?= $aeR ?></small>
                                        </div>
                                        <div>
                                            <label>Answer Arabic</label>
                                            <textarea name="answer_arabic" id="answer_arabic" class="form-textarea h-20"
                                                placeholder="Enter Answer Arabic"><?= $old['answer_arabic'] ?? '' ?></textarea>
                                            <small style="color: #b91c1c;" class="mt-1"><?= $aaR ?></small>
                                        </div>
                                    </div>
                                    <div class="flex justify-between items-center" style='margin-top:100px;'>
                                        <label class="mb-0">
                                            <input type="checkbox" name="status" class="form-checkbox" value="1" <?= (isset($old['status']) && $old['status']) ? 'checked' : '' ?? ''?> />
    ...
    <div x-data="editModal" x-show="showEditModal" x-init="init()">
        <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto items-center justify-center"
            :class="showEditModal && '!block'"
            >

            <div class="flex items-center justify-center min-h-screen px-4">
                <div class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-5xl my-8 text-black dark:text-white-dark bg-white animate__animated"
                    :class="$store.app.rtlClass === 'rtl' ? 'animate__fadeInRight' : 'animate__fadeInLeft'">
                    <!-- Modal Header -->
                    <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                        <h5 class="font-bold text-lg">Edit Product FAQ'S</h5>
                        <button type="button" class="text-white-dark hover:text-dark" @click="showEditModal = false; window.location.reload();">
                        ...
                        <form action="./action.php" method="POST">
                            ...
                            <div>
                                ...
                                <input 
                                    type="text" 
                                    name="title" 
                                    <?php if($editData && $editData['title']){ ?>
                            ...
                            <div class="grid grid-cols-2 w-full mt-3 gap-3">
                                <div>
                                    ...
                                    <input 
                                        ...
                                        <?php if($editData && $editData['question_english']){ ?>
                                        value = "<?php echo $editData['question_english']?>"
                                        <?php }else{?>
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        <?php if($editData && $editData['question_arabic']){ ?>
                                        value = "<?php echo $editData['question_arabic']?>"
                                        <?php }else{?>
                                ...
                                <div>
                                    ...
                                    <input 
                                        type="text" 
                                        name="answer_english" 
                                        <?php if($editData && $editData['answer_english']){ ?>
                                        value = "<?php echo $editData['answer_english']?>"
                                        <?php }else{?>
                                        :value= 'formData.answer_english'
                                        <?php }?>
                                        class="form-textarea !h-20 !resize !overflow-y-auto"
                                    />
                                    <small style="color: #b91c1c;" class="mt-1"><?= $aeR ?></small>
                                </div>
                                <div>
                                    ...
                                    <input 
                                        type="text" 
                                        name="answer_arabic" 
                                        <?php if($editData && $editData['answer_arabic']){ ?>
                                        value = "<?php echo $editData['answer_arabic']?>"
                                        <?php }else{?>
                                        :value= 'formData.answer_arabic'
                                        <?php }?>
                                        class="form-textarea !h-20 !resize !overflow-y-auto"
                                    />
                                    <small style="color: #b91c1c;" class="mt-1"><?= $aaR ?></small>
                            ...
                            <div class="flex justify-between items-center" style='margin-top:100px;'>
                                ...
                                <button type="submit" name="save"
                                    class="btn btn-primary ltr:ml-4 rtl:mr-4">Update</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <div x-data="{ showViewModal: false, editData: {} }" x-init="window.viewData = (data) => { 
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                <div class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-5xl my-8 text-black dark:text-white-dark bg-white animate__animated"
                    ...
                    <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                        ...
                        <button type="button" class="text-white-dark hover:text-dark"
                            @click="showViewModal = false; window.location.reload();">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
                                stroke-linejoin="round" class="w-6 h-6">
                                <line x1="18" y1="6" x2="6" y2="18"></line>
                    ...
                    <div class="p-5">
                        <form action="./action.php" method="POST">
                            <input disabled type="hidden" name="id" :value="editData.id">
                            <div>
                                <label>Title</label>
                                <input disabled type="text" class="form-input" name="title" :value="editData.title"
                            ...
                            <div class="grid grid-cols-2 w-full mt-3 gap-3">
                                <div>
                                    <label>Question Englsih</label>
                                    <input disabled type="text" class="form-input" name="question_english"
                                        :value="editData.question_english" placeholder="Enter Question English">
                                    <small style="color: #b91c1c;" class="mt-1"><?= $qeR ?></small>
                                </div>
                                <div>
                                    <label>Question Arabic</label>
                                    <input disabled type="text" class="form-input" name="question_arabic"
                                        :value="editData.question_arabic" placeholder="Enter Question Arabic">
                                    <small style="color: #b91c1c;" class="mt-1"><?= $qaR ?></small>
                                </div>
                                <div>
                                    <label>Answer Englsih</label>
                                    <textarea disabled name="answer_english" id="answer_english"
                                        class="form-textarea h-20" placeholder="Enter Answer English"
                                        :value="editData.answer_english"></textarea>
                                    <small style="color: #b91c1c;" class="mt-1"><?= $aeR ?></small>
                                </div>
                                <div>
                                    <label>Answer Arabic</label>
                                    <textarea disabled name="answer_arabic" id="answer_arabic"
                                        class="form-textarea h-20" placeholder="Enter Answer Arabic"
                                        :value="editData.answer_arabic"></textarea>
                                    <small style="color: #b91c1c;" class="mt-1"><?= $aaR ?></small>
                                </div>
                            </div>
                            <div class="flex justify-between items-center" style='margin-top:100px;'>
                                <label class="mb-0">
                                    <input disabled type="checkbox" name="status" class="form-checkbox"
                                        x-model="editData.status" :checked="editData.status == 1" />
                                    <span class="mb-0">Status</span>
                                </label>
                                <button type='button' class="btn btn-primary ltr:ml-4 rtl:mr-4"
                                    @click="showViewModal = false">Close</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('bulkDeleteBtn').addEventListener('click', function() {
            ...
            form.method = 'POST';
            form.action = '/php/pim/pages/productFaqs/delete.php';
