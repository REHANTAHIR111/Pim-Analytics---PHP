...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    if (!$question_english) $errors[] = 'Question English, ';
    if (!$question_arabic) $errors[] = 'Question Arabic, ';
    if (!$answer_english) $errors[] = 'Answer English, ';
    if (!$answer_arabic) $errors[] = 'Answer Arabic';
    
    if (!$title) {
        $tiR = 'Please enter Title';
    }
    if (!$question_english) {
        $qeR = 'Please enter Question English';
    }
    if (!$question_arabic) {
        $qaR = 'Please enter Question Arabic';
    }
    if (!$answer_english) {
        $aeR = 'Please enter Answer English';
    }
    if (!$answer_arabic) {
        $aaR = 'Please enter Answer Arabic';
    }
...
//add entry in database//
    if (isset($_POST['save'])) {
        ...
        if ($tiR === '' && $qeR === '' && $qaR === '' && $aeR === '' && $aaR === '') {
            ...
            $showModal = true; // Validation failed, keep modal open
        }
    }
