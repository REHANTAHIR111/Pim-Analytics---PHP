...
<body>
    <form action="./update.php" class='form.ajax-form' method='POST'>
        <?php if (!empty($errors)): ?>
        <div id="error-message" class="flex items-start justify-center p-6 rounded text-danger bg-danger dark:bg-danger-dark-light fixed z-10 w-80 shadow-md shadow-danger" style='right:12px; min-height: 70px; top:70px;'>
            <div class="flex text-white gap-2">
                <div>
                    <?= htmlspecialchars(implode(', ', array_filter($errors))) ?>
                </div>
            </div>
        ...
        <div class="flex xl:flex-row flex-col gap-2">
                    ...
                    <div class='grid grid-cols-2 gap-3'>
                        <div class="">
                            ...
                            <input id="fname" type="text" name="fname" class='form-input disabled:bg-gray-100'
                                ...
                                value="<?= htmlspecialchars($old['fname'] ?? $user['first_name'] ?? '') ?>" />
                                <small style='color: #b91c1c;' class="mt-1">
                                    <?= htmlspecialchars($errors['fnR'] ?? '') ?>
                                </small>
                        ...
                        <div class="">
                            ...
                            <input id="lname" type="text" name="lname" class='form-input disabled:bg-gray-100'
                                ...
                                value="<?= htmlspecialchars($old['lname'] ?? $user['last_name'] ?? '') ?>" />
                            <small style='color: #b91c1c;' class="mt-1">
                                <?= htmlspecialchars($errors['lnR'] ?? '') ?>
                            </small>
                        </div>
                        <div class="">
                            ...
                            <input id="phn" type="number" name="phn" class='form-input disabled:bg-gray-100'
                                ...
                                value="<?= htmlspecialchars($old['phn'] ?? $user['phone_number'] ?? '') ?>" />
                            <small style='color: #b91c1c;' class="mt-1">
                                <?= htmlspecialchars($errors['pnR'] ?? '') ?>
                                <?= htmlspecialchars($errors['vpn'] ?? '') ?>
                                <?= htmlspecialchars($errors['ex2'] ?? '') ?>
                            </small>
                        </div>
                        <div class="">
                            ...
                            <small style='color: #b91c1c;' class="mt-1">
                                <?= htmlspecialchars($errors['emR'] ?? '') ?>
                                <?= htmlspecialchars($errors['vem'] ?? '') ?>
                                <?= htmlspecialchars($errors['ex1'] ?? '') ?>
                            </small>
                        </div>
                        <div class="">
                            ...
                            <input id="dob" type="date" name="dob" class='form-input disabled:bg-gray-100'
                                ...
                                value="<?= htmlspecialchars($old['dob'] ?? $user['date_of_birth'] ?? '') ?>" />
                        </div>
                        <div class="">
                           ...
                           <select id="gender" class="form-select" name="gender">
                                <option value="" disabled <?= empty($old['gender']) && empty($user['gender']) ? 'selected' : '' ?>>
                                    Select Gender
                                </option>
                                <option value="1" <?= (isset($old['gender']) && $old['gender'] == '1') || (!isset($old['gender']) && isset($user['gender']) && $user['gender'] == '1') ? 'selected' : '' ?>>
                                    Male
                                </option>
                                <option value="2" <?= (isset($old['gender']) && $old['gender'] == '2') || (!isset($old['gender']) && isset($user['gender']) && $user['gender'] == '2') ? 'selected' : '' ?>>
                                    Female
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="">
                            ...
                            <input id="cpassword" type="password" name="cpassword" class='form-input disabled:bg-gray-100'
            ...
            <div class="xl:w-96 w-full xl:mt-0 mt-6">
                <div class="panel mb-3">
                    ...
                    <label class="mt-5 mb-0">
                        <input type="checkbox" name="status" class="form-checkbox" value="1"
                            <?= (isset($old['status']) && $old['status'] == '1') || (!isset($old['status']) && isset($user['status']) && $user['status'] == 1) ? 'checked' : '' ?> />
                ...
                <div class="panel">
                    ...
                    <select name="selectRole" class="form-select" id="selectRole">
                        <option value="">Select Role</option>
                        <?php foreach ($roleOptions as $role): ?>
                            <option value="<?= $role['id'] ?>"
                                <?= (
                                    (isset($old['selectRole']) && $old['selectRole'] == $role['id']) || 
                                    (!isset($old['selectRole']) && isset($user['role']) && $user['role'] == $role['id'])
                                ) ? 'selected' : '' ?>>
