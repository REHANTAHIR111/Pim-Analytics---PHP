<?php
    ...
    $query = "SELECT tags.id AS id, tags.name AS name, tags.name_ar AS name_ar, tags.image_link_app AS image_link, tags.icon AS icon, GROUP_CONCAT(sub_tags.name SEPARATOR ', ') AS subtags, tags.sorting AS sorting, tags.status AS status FROM tags LEFT JOIN sub_tags ON sub_tags.tag_id = tags.id $condition GROUP BY tags.id ORDER BY tags.id DESC";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $tags_icon = $row['icon'];
        $subtags = $row['subtags'] ?? '<div class="text-danger font-bold">No Sub Tags</div>';
        $sorting = $row['sorting'];
        ...
        if ($delete){
            $action .= '<a href="/php/pim/pages/tags/delete.php?id=' . $row['id'] . '">
        ...
        $users[] = [$id, $tags_name, $subtags, $sorting, $status, $action];
    }
?>
...
<div x-data="custom">
    <div class="panel">
        <div class="flex gap-3 items-center absolute top-5">
            ...
            >
                <?php if ($create) : ?>
                    <button @click="showAddModal=true" class="btn btn-dark">Add New Tag</button>
                <?php endif; ?>
                <!-- Modal Backdrop -->
                    ...
                    <div class="flex items-center justify-center min-h-screen px-4">
                        ...
                        >
                            ...
                            <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                                <h5 class="font-bold text-lg">Add New Tag</h5>
        ...
        <div 
            ...
            <div class="flex items-center justify-center min-h-screen px-4" >
                <div 
                    ...
                    <!-- Modal Header -->
                    <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                        <h5 class="font-bold text-lg">Edit Tags</h5>
                        ...
                        <form action="./action.php" method="POST">
                            ...
                            <div class="grid grid-cols-2 w-full gap-3">
                                <div>
                                    ...
                                    <input 
                                        type="text" 
                                        name="tagNameEn" 
                                        <?php if($editData && $editData['tagNameEn']){ ?>
                                        value = "<?php echo $editData['tagNameEn']?>"
                                        <?php }else{?>
                                        :value= 'formData.name'
                                        <?php }?>
                                        class="form-input"
                                ...
                                <div>
                                    ...
                                    <input 
                                        type="text" 
                                        name="tagNameAr" 
                                        <?php if($editData && $editData['tagNameAr']){ ?>
                                        value = "<?php echo $editData['tagNameAr']?>"
                                        <?php }else{?>
                            ...
                            <div class="w-full mt-3">
                                ...
                                <input 
                                    type="text" 
                                    name="sorting" 
                                    <?php if($editData && $editData['sorting']){ ?>
                                    value = "<?php echo $editData['sorting']?>"
                                    <?php }else{?>
                            ...
                            <div class="w-full mt-3">
                                ...
                                <input 
                                    type="text" 
                                    name="uploadIcon" 
                                    <?php if($editData && $editData['uploadIcon']){ ?>
                                    value = "<?php echo $editData['uploadIcon']?>"
                                    <?php }else{?>
                                    :value= 'formData.icon'
                                    <?php }?>
                                    class="form-input"
                            ...
                            <div class="w-full mt-3 mb-16">
                                ...
                                <input 
                                    type="text" 
                                    name="imageLink" 
                                    <?php if($editData && $editData['imageLink']){ ?>
                                    value = "<?php echo $editData['imageLink']?>"
                                    <?php }else{?>
                ...
                <div class="p-5">
                    <form action="./action.php" method="POST">
                        ...
                        <div class="grid grid-cols-2 w-full gap-3">
                            <div>
                                <label>Tag Name - En</label>
                                <input type="text" class="form-input" name="tagNameEn" disabled :value="editData.name" placeholder="Tag Name - En">
                                <small style="color: #b91c1c;" class="mt-1"><?= $etnR ?></small>
                            </div>
                            <div>
                                <label>Tag Name - Ar</label>
                                <input type="text" class="form-input" name="tagNameAr" disabled :value="editData.name_ar" placeholder="Tag Name - Ar">
                                <small style="color: #b91c1c;" class="mt-1"><?= $eanR ?></small>
                            </div>
                        </div>
...
<script>
            ...
            init() {
                ...
                const data = this.tableData.map(user => {
                    ...
                    return [
                        ...
                        user[1],
                        `<div style='overflow-x: auto; width: 200px; white-space: nowrap;'>${user[2]}</div>`,
                        user[3],
                        user[4],
                        `<button name="a_${id}" x-model="ids"><a href="/php/pim/pages/tags/subtags/?id=${user[0]}" class="btn btn-success">Create Sub Tag</a></button>`,
                        user[5],
                    ];
                });
    ...
    document.getElementById('bulkDeleteBtn').addEventListener('click', function() {
        ...
        form.method = 'POST';
        form.action = '/php/pim/pages/tags/delete.php';
