...
$perm = "SELECT * FROM permission 
         ...
         WHERE role_id = $role_id AND module_id = 4";
$result = mysqli_query($conn, $perm);

$row = mysqli_fetch_assoc($result);

if (!$row) {
    die("❌ No permission entry found for role_id = $role_id and module_id = 4");
}
...
$condition = ($can_view_all == 1) ? "" : "WHERE productcategories.creator_id = $user_id";

$query = "SELECT id, name, slug, sorting, website_image, mobile_image, show_in_menu, status from productcategories $condition ORDER BY productcategories.id DESC";
$result = mysqli_query($conn, $query);

while ($row = mysqli_fetch_assoc($result)) {
    $id = $row['id'];
    $name = $row['name'];
    $slug = basename($row['slug']);
    $sorting = $row['sorting'];
    $image = $row['website_image'];
    $mobile_image = $row['mobile_image'];
    $showInMenu = $row['show_in_menu'] == 0 ? '<div class="text-danger font-semibold">Disabled</div>' : '<div class="text-success font-semibold">Enabled</div>';
    ...
    $action .= '<a href="/php/pim/pages/category/view.php?id=' . $row['id'] . '">
    ...
    if ($edit) {
        $action .= '<a href="/php/pim/pages/category/edit.php?id=' . $row['id'] . '">
    ...
    if ($delete){
        $action .= '<a href="/php/pim/pages/category/delete.php?id=' . $row['id'] . '">
    ...
    $users[] = [$id, $name, $slug ,$sorting, $image, $mobile_image, $showInMenu, $status ,$action];
}
?>