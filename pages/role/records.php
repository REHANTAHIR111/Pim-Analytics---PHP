...
$perm = "SELECT * FROM permission 
         ...
         WHERE role_id = $role_id AND module_id = 1";
...
$condition = ($can_view_all == 1) ? "" : "WHERE role.creator_id = $user_id";

$query = "SELECT id, role from role $condition ORDER BY role.id DESC";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $action .= '<a href="/php/pim/pages/role/view.php?id=' . $row['id'] . '">
    ...
    if ($edit) {
        $action .= '<a href="/php/pim/pages/role/edit.php?id=' . $row['id'] . '">
    ...
    if ($delete){
        $action .= '<a href="/php/pim/pages/role/delete.php?id=' . $row['id'] . '">
